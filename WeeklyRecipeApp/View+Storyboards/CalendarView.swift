...
class CalenderView: UIView, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, MonthViewDelegate, CalendarViewControllerDelegate {
        ...
        if indexPath.item <= firstWeekDayOfMonth - 2 {
            ...
            if calcDate < todaysDate && currentYear == presentYear && currentMonthIndex == presentMonthIndex {
                ...
                if cell.day?.recipes?.count != 0 {
                    cell.backgroundColor = UIColor.newOrange.withAlphaComponent(0.5)
                }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if cell?.backgroundColor == UIColor.newOrange.withAlphaComponent(0.5) || cell?.backgroundColor == UIColor.newOrange.withAlphaComponent(0.9) {
            cell?.backgroundColor = UIColor.newOrange.withAlphaComponent(0.9)
        } else {
            cell?.backgroundColor = UIColor.newOrange
        }
        isSelected = true
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didDeselectItemAt indexPath: IndexPath) {
        ...
        if cell?.backgroundColor == UIColor.newOrange.withAlphaComponent(0.9) {
            cell?.backgroundColor = UIColor.newOrange.withAlphaComponent(0.5)
        } else {
            cell?.backgroundColor = UIColor.clear
        }
